...
# --- ADVANCED CUSTOM CSS (Animated Background & Fonts) ---
st.markdown("""
    <style>
    /* 1. Import Custom Tech Font from Google Fonts */
    @import url('https://fonts.googleapis.com/css2?family=Rajdhani:wght@400;500;600;700&display=swap');

    /* 2. Force Rajdhani on EVERY text element so the sleek look returns */
    html, body, [class*="css"], [class*="st-"], span, div, p, h1, h2, h3, h4, h5, h6, a, button, input {
        font-family: 'Rajdhani', sans-serif !important;
    }

    /* 3. PROTECT the Streamlit icons (like dropdown arrows) so they don't break */
    .material-icons, 
    ...
    /* This dictates how the background moves */
    ...
    /* Main padding: Added top padding to clear the Streamlit header */
...
if not st.session_state.logged_in:
    ...
    with col1:
        if st.button("Login as Admin", use_container_width=True):
            if username == "admin" and password == "admin123": # Hardcoded credentials for the demo
...
# ==========================================
    ...
    # RBAC LOGIC: Check if the user is a guest to disable controls
    ...
    with st.sidebar:
        ...
        if is_guest:
            st.warning("👤 Logged in as: **GUEST** (View Only)")
        else:
        ...
        with st.expander("🔌 Active Endpoints", expanded=True):
            laptop_on = st.toggle("💻 Work Laptop (Priority)", value=True, disabled=is_guest)
    ...
    with tab1:
        ...
        with col1:
            st.metric(label="Total Uplink Capacity", value=f"{total_bw} Mbps")
        with col2:
            st.metric(label="Active Endpoints", value=len(active_df))
        with col3:
            if not active_df.empty and 'Work Laptop' in active_df['name'].values:
                ...
                st.metric(label="Critical Node Health", value=status, delta="Optimal" if status == "Secure" else "-Warning", delta_color="normal" if status == "Secure" else "inverse")
            else:
                st.metric(label="Critical Node Health", value="Standby")
    ...
    with tab2:
        ...
        if not ai_mode:
            st.warning("⚠️ Neural Forecast is currently disabled. Activate it in the sidebar to view analytics.")
        else:
            st.info("The AI engine uses Random Forest Regression to analyze historical congestion patterns and autonomously pre-throttle non-critical nodes.")
            
            if predicted_demand > total_bw:
                st.error(f"🔴 **CRITICAL ALERT:** Projected demand at {simulated_hour}:00 is **{predicted_demand} Mbps**. Physical capacity is only {total_bw} Mbps. Low-tier devices have been quarantined.")
            else:
                st.success(f"🟢 **SYSTEM STABLE:** Projected demand at {simulated_hour}:00 is **{predicted_demand} Mbps**. No automated intervention required.")
            
            st.markdown("#### Simulated 24-Hour Demand Curve")
            ...
            fig2.add_hline(y=total_bw, line_dash="dash", line_color="red", annotation_text="ISP Capacity Limit")
            fig2.update_layout(
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", 
    ...
    with tab3:
        ...
        st.dataframe(
            df[['name', 'category', 'weight', 'active', 'allocated']],
            column_config={
                "name": "Endpoint Name", "category": "QoS Tier", "weight": "Algorithm Weight",
                "active": "Connection Status", "allocated": st.column_config.NumberColumn("Current Allocation (Mbps)", format="%.2f")
            },
